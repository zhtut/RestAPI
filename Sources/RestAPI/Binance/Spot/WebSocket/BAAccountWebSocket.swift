...
open class BAAccountWebSocket: BAWebSocket {
    
    public static let shared = BAAccountWebSocket()
    ...
    open func refreshListenKey() async {
        ...
        if result.succ {
            log("ListenKey请求成功，开始连接websocket")
            self.open()
            
            log("开始预订刷新ListenKey的有效期")
            scheduledPutListenKey()
        } else {
    ...
    func putListenKey() async {
        let path = "PUT /api/v3/userDataStream"
